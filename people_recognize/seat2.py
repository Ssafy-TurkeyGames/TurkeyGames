...
stable_time_threshold = 1  # 상태 변화가 있어야 로그를 출력하는 시간 (초)
...
while True:
    ...
    for seat_num in aruco_markers:
        if seat_num in seat_occupancy:
            # 사람이 앉은 상태에서 일정 시간 이상 변화가 없으면 로그 출력
            if current_time - seat_last_change_time[seat_num] > stable_time_threshold:
                print(f"좌석 {seat_num}에 사람이 앉아 있습니다.")
                seat_last_change_time[seat_num] = current_time  # 로그 시간 기록
        else:
            # 좌석이 비었을 때도 일정 시간 후 상태 변화 확인
            if seat_last_change_time[seat_num] is not None and current_time - seat_last_change_time[seat_num] > stable_time_threshold:
