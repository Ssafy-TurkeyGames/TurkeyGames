...
# 마우스 좌표 변수
mouse_x, mouse_y = -1, -1
norm_x, norm_y = None, None  # 상대좌표 변수 추가
        ...
        if len(ids) == 4:
            ...
            if mouse_x >= 0 and mouse_y >= 0:
                ...
                if is_inside:
                    ...
                    norm_x, norm_y = dst_pt[0], dst_pt[1]
                    ...
                    # 상대좌표 표시 (화면)
                    cv2.putText(frame, f"상대좌표: ({norm_x:.3f}, {norm_y:.3f})", 
                                (mouse_x+10, mouse_y+10),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
                    
                    # 화면 상단에 큰 글씨로 상대좌표 표시 (태스크 4 추가)
                    cv2.putText(frame, f"직사각형 내 좌표: x={norm_x:.3f}, y={norm_y:.3f}", 
                                (50, 180),
                                cv2.FONT_HERSHEY_SIMPLEX, 1.0, (255, 0, 255), 2, cv2.LINE_AA)
                    
                    # 콘솔에 상대좌표 출력 (태스크 4 추가)
                    print(f"[INFO] 직사각형 내 상대좌표: x={norm_x:.3f}, y={norm_y:.3f}")
                else:
                    # 외부에 있을 때는 파란색으로 표시
